...
//----------------------------------------------------------------------------------------------------------------------------------------------
//GLOBAL 変数用設定
$top_page = 'https://higuchikiyoto.github.io/portfolio/';																			//トップページURL（送信完了後リンク先）
$title_arr = ['name'=>'氏名','mail'=>'メールアドレス','tell'=>'電話番号','text'=>'件名','contact'=>'お問い合わせ内容'];	//項目・項目名セット
$req_items = ['name'=>'req','mail'=>'req','tell'=>'req','text'=>'req','contact'=>'req'];							//必須項目設定（reqは必須、''は任意
$err_message = resetItems($req_items);																				//エラーメッセージ用（！編集非推奨！）
// text => 記号禁止、mail => メールアドレス形式チェック、tel =>　電話番号形式チェック 
$val_items = ['name'=>'text','mail'=>'mail','tell'=>'tel','text'=>'text','contact'=>'text'];						//validate方法選択
$hankaku_array = array('tell','mail');																				//半角化に変換処理を行う、POSTデータ（各nameを記述）
//メール設定
$to_mail = 'kiyoto.1995.1127@softbank.ne.jp';										//送信先（管理者宛）メールアドレス(to)
